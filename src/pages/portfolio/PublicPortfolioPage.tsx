  ...
  return (
    <div className={`min-h-screen ${getThemeClass()} selection:bg-amber-500 selection:text-black`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-12 items-start">
          <div className="space-y-6">
            ...
            <p className="text-zinc-400 max-w-2xl leading-relaxed text-base">
              {profile.Summary || "Bio / Professional summary goes here."}
            </p>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-24">
                  ...
                  <div className="p-6 space-y-4">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        {project.LiveDemoUrl && (
                          <a href={project.LiveDemoUrl} target="_blank" rel="noreferrer" className="text-zinc-400 hover:text-white">
              ...
              <div className="space-y-8 border-l border-white/5 pl-4 ml-2">
                {experiences.map((exp: any) => (
                  <div key={exp.Id} className="relative space-y-2">
                    ...
                    <div className="text-sm font-medium text-zinc-400">{exp.CompanyName} • {exp.EmploymentType}</div>
        ...
        {/* Certifications & Languages & Custom Sections */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              ...
              <div className="space-y-4">
                {certifications?.map((cert: any) => (
                  <div key={cert.Id} className={`p-4 rounded-xl ${getCardBg()} flex justify-between items-center`}>
                    <div>
                      ...
                      <div className="text-xs text-zinc-400">{cert.IssuingOrganization}</div>
                    </div>
                    {cert.CertificateUrl && (
                      <a href={cert.CertificateUrl} target="_blank" rel="noreferrer" className="text-amber-500 hover:underline text-xs flex items-center gap-1">
                ...
                {achievements?.map((ach: any) => (
                  <div key={ach.Id} className={`p-4 rounded-xl ${getCardBg()} space-y-1`}>
                    ...
                    {ach.Issuer && <div className="text-xs text-zinc-400">{ach.Issuer}</div>}
              ...
              <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {languages.map((lang: any) => (
                        <span key={lang.Id} className="text-xs px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                          {lang.LanguageName} <span className="text-amber-500/80">({lang.Proficiency})</span>
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                {customSections?.map((section: any) => (
                  <div key={section.Id} className="space-y-2">
                    ...
                    <p className="text-sm text-zinc-400 leading-relaxed white-space-pre-wrap">{section.Content}</p>
                  </div>
                ))}
              </div>
            </section>
